...
def take_action(state, action):
  ...
  if i < 0 or i >= rows or j < 0 or j >= cols or maze[i][j] == "#":
    ...
    return (i,j), 10   # reward for reaching the end

  else:
    return (i,j), -1   #small penalty to find the shortest route
...
episodes = 1000   # the amount of times the agent runs through the maze to learn
